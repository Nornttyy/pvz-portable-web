...
启动页提供资源重选、简短首次使用说明、网址复制和断网提示。存档读取超过 15 秒会停止启动并显示重试，不会以空进度覆盖已有存档；保存超时可手动备份。网页组件加载失败有独立提示，所有入口脚本、样式、模块与 WASM 使用同一构建版本号，避免刷新后混用新旧代码。手机横竖屏变化会重新计算画面尺寸并避开屏幕安全区域。
